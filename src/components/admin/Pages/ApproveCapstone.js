  ...
  const onSubmitForm = async (e) => {
    ...
    try {
      ...
      if (
        ...
        const add_research_response = await fetch(
        ...
        const research_Data = await add_research_response.json();
        console.log(research_Data);

        const id = research_Data.research_id;

        if (add_research_response.status === 200) {
...
export default ApproveCapstone;
